...
// Settings
import { CreateTenantPage } from './pages/system/CreateTenantPage';
import { TenantListPage } from './pages/system/TenantListPage';
...
const TenantGuard = ({ children }: { children: React.ReactNode }) => {
  const [status, setStatus] = React.useState<'LOADING' | 'ACTIVE' | 'INACTIVE'>('LOADING');

  React.useEffect(() => {
    const checkTenant = async () => {
      try {
        const domain = getTenantId();
        const res = await fetch(`${API_BASE_URL}/api/system/tenant-status/${domain}`);
        if (res.ok) {
          const data = await res.json();
          setStatus(data.status);
        } else {
          // If not found or error, just allow to proceed and let other mechanisms handle it
          setStatus('ACTIVE');
        }
      } catch (err) {
        setStatus('ACTIVE');
  ...
  if (status === 'LOADING') {
    return <div className="h-screen w-screen flex items-center justify-center bg-slate-50"><p className="text-slate-500">Checking workspace...</p></div>;
  }
    ...
    return (
      <div className="h-screen w-screen flex items-center justify-center bg-slate-50 p-6">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl border border-slate-100 p-8 text-center">
          ...
          <h1 className="text-2xl font-bold text-slate-900 mb-2">Workspace Inactive</h1>
          <p className="text-slate-600 mb-8">
            This page is inactive. Please contact PMJ services to reactivate your workspace and resume access.
          </p>
          <a href="mailto:manoj20062707@gmail.com" className="inline-flex items-center justify-center px-6 py-2.5 bg-indigo-600 text-white rounded-lg font-medium hover:bg-indigo-700 transition-colors">
            Contact Support
          </a>
        </div>
      </div>
      ...
      <BrowserRouter>
        <Routes>
          ...
          <Route path="/" element={<ProtectedRoute><DashboardLayout /></ProtectedRoute>}>
            ...
            {/* Settings Routes */}
            <Route path="system/tenants/create" element={<CreateTenantPage />} />
            <Route path="system/tenants/list" element={<TenantListPage />} />
