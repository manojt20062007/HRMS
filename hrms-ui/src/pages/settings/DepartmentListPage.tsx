...
export const DepartmentListPage = () => {
  ...
  const fetchDepartments = async () => {
    setLoading(true);
    try {
      const response = await fetch('http://localhost:3001/api/settings/departments', {
  ...
  const handleAdd = async () => {
    ...
    setSaving(true);
    try {
      const res = await fetch('http://localhost:3001/api/settings/departments', {
        method: 'POST',
  ...
  const handleUpdate = async (id: string) => {
    ...
    try {
      const res = await fetch(`http://localhost:3001/api/settings/departments/${id}`, {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json', 'x-tenant-id': 'pmj.com' },
  ...
  return (
    <div className="max-w-[1600px] mx-auto pb-10">
      <div className="flex justify-between items-start mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white mb-1 tracking-tight">Department List</h1>
          <div className="flex items-center gap-2 text-sm">
            <span className="text-emerald-500 font-medium">Employee Configuration</span>
            <span className="text-slate-300">›</span>
            <span className="text-slate-500">Department List</span>
          </div>
        </div>
        <button
          onClick={() => navigate(-1)}
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm text-slate-600 dark:text-slate-300">
            ...
            <tbody className="divide-y divide-border">
              {loading ? (
                <tr><td colSpan={3} className="px-6 py-8 text-slate-500 text-center font-medium">Loading departments...</td></tr>
              ) : departments.length === 0 ? (
                <tr><td colSpan={3} className="px-6 py-8 text-slate-500 text-center font-medium">No departments found. Click "Add Department" to create one.</td></tr>
              ) : (
                departments.map((row) => (
          ...
          <div className="flex justify-between items-center text-sm text-slate-500 p-4 border-t border-border">
            <span>Showing {departments.length} of {departments.length} entries</span>
