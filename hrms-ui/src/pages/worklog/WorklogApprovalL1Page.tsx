...
export const WorklogApprovalL1Page = () => {
  ...
  const fetchPendingWorklogs = async () => {
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/worklog?managerId=${user.employeeId}&status=PENDING_L1`, {
      ...
      console.error('Failed to fetch pending worklogs', error);
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="max-w-[1600px] mx-auto pb-10">
      <PageHeader 
        title="Worklog Approval (L1)" 
        breadcrumbs={['Worklog & Travel', 'Worklog Approval L1']} 
      />

      <div className="bg-white dark:bg-card border border-border shadow-sm rounded-xl overflow-hidden mt-6">
        <div className="p-4 border-b border-border bg-slate-50/50">
          <h3 className="text-lg font-bold text-slate-800 dark:text-white">Pending Approvals from Team</h3>
        </div>
          ...
          <table className="w-full text-sm text-left text-slate-600 dark:text-slate-300">
            ...
            <tbody className="divide-y divide-border">
              {loading ? (
                ...
                <tr><td colSpan={5} className="px-6 py-8 text-center text-slate-500">No pending worklogs from your team.</td></tr>
              ) : (
                worklogs.map(log => (
                  <tr key={log.id} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right">
                      <div className="flex items-center justify-end gap-2">
                        <button 
                          onClick={() => handleUpdateStatus(log.id, 'PENDING_L2')}
