...
export const StaffingListHRPage = () => {
  ...
  return (
    <div className="max-w-[1600px] mx-auto pb-10">
      <PageHeader 
        title="Staffing Request List HR" 
        breadcrumbs={['Recruitment', 'Staffing Request List HR']} 
      />
      ...
      <div className="bg-white dark:bg-card border border-border shadow-sm rounded-xl overflow-hidden mt-6">
        <div className="p-4 border-b border-border bg-gradient-to-r from-blue-100/50 to-emerald-100/50 dark:from-blue-900/20 dark:to-emerald-900/20 flex justify-between items-center">
          <div className="flex items-center gap-2 text-indigo-900 dark:text-indigo-300 font-semibold">
            <UserCheck className="h-5 w-5" /> HR Approval Queue
          </div>
        </div>
          ...
          <table className="w-full text-sm text-left text-slate-600 dark:text-slate-300">
            ...
            <tbody className="divide-y divide-border">
              {loading ? (
                ...
                <tr><td colSpan={6} className="px-6 py-8 text-center text-slate-500">No pending requests for HR approval</td></tr>
              ) : (
                requests.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50 dark:hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-2">
                        <button onClick={() => handleUpdateStatus(row.id, 'PENDING_CTO')} className="flex items-center justify-center w-8 h-8 rounded-full bg-emerald-50 text-emerald-600 hover:bg-emerald-100 transition-colors tooltip" title="Approve">
