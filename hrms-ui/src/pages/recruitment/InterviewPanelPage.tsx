...
import { Users, Play, XCircle } from 'lucide-react';

export const InterviewPanelPage = () => {
  const [candidates, setCandidates] = useState<any[]>([]);
  const [loading, setLoading] = useState(true);

  const fetchCandidates = async () => {
    try {
      const response = await fetch('http://localhost:3001/api/recruitment/candidates?status=INTERVIEW_PANEL', {
        headers: {  }
      });
      if (response.ok) {
  ...
  return (
    <div className="max-w-[1600px] mx-auto pb-10">
      <PageHeader 
        ...
        breadcrumbs={['Recruitment', 'Interview Panel']} 
      />

      <div className="bg-white dark:bg-card border border-border shadow-sm rounded-xl overflow-hidden mt-6">
        <div className="p-4 border-b border-border bg-gradient-to-r from-indigo-100/50 to-purple-100/50 dark:from-indigo-900/20 dark:to-purple-900/20 flex items-center gap-2 text-indigo-900 dark:text-indigo-300 font-semibold">
          <Users className="h-5 w-5" /> Interview Panel
        </div>
          ...
          <table className="w-full text-sm text-left text-slate-600 dark:text-slate-300">
            <thead className="text-[11px] uppercase bg-[#F4F7FE] dark:bg-muted text-slate-700 dark:text-slate-200">
              <tr>
                ...
                <th className="px-6 py-4 font-semibold text-center">Action (Move to CAF)</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {loading ? (
                ...
                <tr><td colSpan={5} className="px-6 py-8 text-center text-slate-500">No candidates in interview panel</td></tr>
              ) : (
                candidates.map((row) => (
                  <tr key={row.id} className="hover:bg-slate-50 dark:hover:bg-muted/30 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center justify-center gap-2">
                        <button onClick={() => handleUpdateStatus(row.id, 'CAF')} className="flex items-center justify-center w-8 h-8 rounded-full bg-indigo-50 text-indigo-600 hover:bg-indigo-100 transition-colors tooltip" title="Move to CAF">
