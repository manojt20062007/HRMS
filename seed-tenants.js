const { PrismaClient } = require('./hrms-api/node_modules/@prisma/client');
const { exec } = require('child_process');
const util = require('util');
const bcrypt = require('./hrms-api/node_modules/bcrypt');

const execPromise = util.promisify(exec);
const systemPrisma = new PrismaClient({
  datasources: {
    db: {
      url: "postgresql://neondb_owner:npg_JX9YbTUriC3z@ep-long-sea-azgefq0j-pooler.c-3.ap-southeast-1.aws.neon.tech/neondb?sslmode=require&channel_binding=require"
    }
  }
});
...
async function provisionTenant(name, domain, schemaName) {
  ...
  // Also seed default employee roles
